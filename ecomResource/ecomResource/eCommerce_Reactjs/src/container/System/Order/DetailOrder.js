        ...
        if (id) {
            let fetchOrder = async () => {
                ...
                if (order && order.errCode == 0) {
                    ...
                    setpriceShip(order.data.type_ship_data.price)
                }
            }
            fetchOrder()
            ...
            <div className="wrap-order">
                            ...
                            {DataOrder && DataOrder.addressUser &&
                                <>
                                    <div className="content-left">
                                        ...
                                        <span>{DataOrder.addressUser.ship_name} ({DataOrder.addressUser.ship_phonenumber})</span>
                    ...
                    <section className="cart_area">
                        <div className="container">
                                        ...
                                        <tbody>
                                            ...
                                            {DataOrder.orderDetail && DataOrder.orderDetail.length > 0 &&
                                                DataOrder.orderDetail.map((item, index) => {
                                                    price += item.quantity * item.product_detail.discountPrice

                                                    let name = `${item.product.name} - ${item.product_detail.nameDetail} - ${item.product_detail_size.sizeData.value}`
                                                    return (
                                                        <ShopCartItem isOrder={true} id={item.id} product_detailsizeId={item.product_detail_size.id} key={index} name={name} price={item.product_detail.discountPrice} quantity={item.quantity} image={item.productImage[0].image} />
                                                    )
                                                })
                            ...
                            <div className="box-shipping">
                                ...
                                <div>
                                    {
                                        DataOrder && DataOrder.type_ship_data &&
                                        <label className="form-check-label">{DataOrder.type_ship_data.type} - {CommonUtils.formatter.format(DataOrder.type_ship_data.price)} </label>
                                    }
                ...
                <div className="wrap-payment">
                    <div className="content-top" style={{ display: 'flex', gap: '10px' }}>
                        ...
                        <div className='box-type-payment active'>{DataOrder.is_payment_online == 0 ? 'Thanh toán tiền mặt' : 'Thanh toán online'}</div>

                    </div>
                    <div className="content-top" style={{ display: 'flex', gap: '10px' }}>
                        <span>Trạng Thái Đơn Hàng</span>
                        <div className='box-type-payment active'>{DataOrder.status_order_data && DataOrder.status_order_data.value}</div>
                    ...
                    <div className="content-bottom">
                        {DataOrder && DataOrder.addressUser &&
                            <div className="wrap-bottom">
                                <div className="box-flex">
                                    ...
                                    <div >{DataOrder.addressUser.ship_name}</div>
                                </div>
                                <div className="box-flex">
                                    <div className="head">Số điện thoại</div>
                                    <div >{DataOrder.addressUser.ship_phonenumber}</div>
                                </div>
                                <div className="box-flex">
                                    <div className="head">Địa chỉ email</div>
                                    <div >{DataOrder.addressUser.ship_email}</div>
                                </div>
                        ...
                        <div className="wrap-bottom">
                            ...
                            <div className="box-flex">
                                {DataOrder && DataOrder.status_id == 'S3' &&
                                ...
                                {DataOrder && DataOrder.status_id == 'S4' &&
                                    <a onClick={() => handleSendProduct()} className="main_btn">Gửi hàng</a>
                                }
                                {DataOrder && DataOrder.status_id == 'S5' &&
                                    <a onClick={() => handleSuccessShip()} className="main_btn">Đã giao hàng</a>
                                }
                            </div>
                            {(DataOrder && DataOrder.status_id == 'S3' && DataOrder.is_payment_online == 0)
                                &&
